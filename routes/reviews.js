...
router.post('/createNewReview', async (req, res) => {
    try {
        ...
        const booking = await db.collection(BOOKING_COLLECTION).doc(reviewDTO.booking_id).get();
        const currentDate = moment.now()
        const targetDateString = booking.data().time;
        // Parse the target date string into a Date object
        const targetDate = moment(targetDateString);
        if (!booking.exists) {
            res.status(404).json(new Message("There is no such booking", null, 0))
        } else if (targetDate < currentDate) {
            res.status(400).json(new Message("Giving a review is available only after the booking has been done.", null, 0));
        } else {
...
router.delete(('/deleteReview'), async (req, res) => {
    try {
        ...
        const reviewSnapshot = await db.collection(REVIEWS_COLLECTION).doc(review_id).get();
        const reviewData = reviewSnapshot.data();
        console.log(reviewData)
        if (_user_id === reviewData._user_id) {
            // User is authorized, proceed with deletion
            await db.collection(REVIEWS_COLLECTION).doc(review_id).delete();
